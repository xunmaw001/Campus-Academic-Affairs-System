<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="教职工号" prop="jiaozhigonghao">
          <el-select  @change="jiaozhigonghaoChange" v-model="ruleForm.jiaozhigonghao" placeholder="请选择教职工号">
            <el-option
                v-for="(item,index) in jiaozhigonghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.jiaozhigonghao" label="教职工号" prop="jiaozhigonghao">
              <el-input v-model="ruleForm.jiaozhigonghao" 
                placeholder="教职工号" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="教室名称" prop="jiaoshimingcheng">
          <el-select v-model="ruleForm.jiaoshimingcheng" placeholder="请选择教室名称">
            <el-option
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="上课时长" prop="shangkeshizhang">
          <el-input v-model="ruleForm.shangkeshizhang" 
              placeholder="上课时长" clearable  :readonly="ro.shangkeshizhang"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="上课时长" prop="shangkeshizhang">
              <el-input v-model="ruleForm.shangkeshizhang" 
                placeholder="上课时长" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.kechengmingcheng = true;
          ...
          if(o=='jiaozhigonghao'){
            this.ruleForm.jiaozhigonghao = obj[o];
	    this.ro.jiaozhigonghao = true;
            continue;
          }
	    ...
	    this.ro.shangkeshijian = true;
          ...
          if(o=='shangkeshizhang'){
            this.ruleForm.shangkeshizhang = obj[o];
	    this.ro.shangkeshizhang = true;
            continue;
          }
        }
      }
    ...
    // 下二随
    jiaozhigonghaoChange () {
      this.$http({
        url: `follow/jiaoshi/jiaoshixingming?columnValue=`+ this.ruleForm.jiaozhigonghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              ...
              if(data.data.kemu){
                this.ruleForm.kemu = data.data.kemu
              }
              if(data.data.shouji){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `wangshangxuanke/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.wangshangxuankeCrossAddOrUpdateFlag = false;
                  this.parent.search();
