  ...
  methods: {
    ...
    init(id,type) {
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          var json = data.data;
		if(json.jiaozhigonghao!=''&&json.jiaozhigonghao){
              		this.ruleForm.jiaozhigonghao = json.jiaozhigonghao
		}
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.renwuleixingOptions = "作业,考试,劳动,发布消息".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `xiaoyuanrenwu/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xiaoyuanrenwu/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xiaoyuanrenwuCrossAddOrUpdateFlag = false;
                  this.parent.search();
