<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="上课星期" prop="shangkexingqi">
          <el-input v-model="ruleForm.shangkexingqi" 
              placeholder="上课星期" clearable  :readonly="ro.shangkexingqi"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="上课星期" prop="shangkexingqi">
              <el-input v-model="ruleForm.shangkexingqi" 
                placeholder="上课星期" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="所属节数" prop="suoshujieshu">
          <el-select v-model="ruleForm.suoshujieshu" placeholder="请选择所属节数">
            <el-option
                v-for="(item,index) in suoshujieshuOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="所属节数" prop="suoshujieshu">
	      <el-input v-model="ruleForm.suoshujieshu"
                placeholder="所属节数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.jiaoshixingming" label="教师姓名" prop="jiaoshixingming">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='shangkexingqi'){
            this.ruleForm.shangkexingqi = obj[o];
	    this.ro.shangkexingqi = true;
            continue;
          }
          if(o=='kemu'){
	    ...
	    this.ro.kemu = true;
          ...
          if(o=='suoshujieshu'){
            this.ruleForm.suoshujieshu = obj[o];
	    this.ro.suoshujieshu = true;
            continue;
          }
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.suoshujieshuOptions = "第一节,第二节,第三节,第四节,第五节,第六节".split(',')
            this.$http({
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `kebiaochaxun/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.kebiaochaxunCrossAddOrUpdateFlag = false;
                  this.parent.search();
