    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="具体评价" prop="jutipingjia">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="具体评价"
                  v-model="ruleForm.jutipingjia" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.jutipingjia" label="具体评价" prop="jutipingjia">
                    <span>{{ruleForm.jutipingjia}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.pingfen = true;
          ...
          if(o=='jutipingjia'){
            this.ruleForm.jutipingjia = obj[o];
	    this.ro.jutipingjia = true;
            continue;
          }
          if(o=='xuehao'){
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.pingfenOptions = "A,B,C".split(',')
    },
    // 下二随
    jiaozhigonghaoChange () {
      this.$http({
        url: `follow/jiaoshi/jiaozhigonghao?columnValue=`+ this.ruleForm.jiaozhigonghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `pingjiaoxiangqing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.pingjiaoxiangqingCrossAddOrUpdateFlag = false;
                  this.parent.search();
